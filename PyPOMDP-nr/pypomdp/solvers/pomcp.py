...
class POMCP(Solver):
    ...
    def update_belief(self, belief, action, obs):
        ...
        if particle_slots > 0:
            ...
            with open('./logs/particles.txt', 'a+') as outfile:
                outfile.writelines(str(particles))
            p_unique = list(set(particles))
            plt.savefig('./logs/particle.png')

            # This is the corresponding count for each value
            counts = [particles.count(value) for value in p_unique]

            barcontainer = plt.barh(range(len(p_unique)),counts)

            # Some labels and formatting to look more like the example
            plt.bar_label(barcontainer,p_unique, label_type='edge')
            plt.axis('off')
