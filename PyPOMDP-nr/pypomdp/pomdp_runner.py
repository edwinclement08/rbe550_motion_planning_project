...
class PomdpRunner:
    ...
    def run(self, algo, T, **kwargs):
        ...
        for i in range(params.max_play):
            ...
            if (i%10):
                plt.plot(belief)
            '''ax = sns.barplot(belief)
            ax.bar_label(ax.containers[0])
            plt.axis('off')'''
            plt.savefig('./logs/belief.png')
            with open('./logs/Belief.txt', 'a+') as outfile:
                outfile.writelines(str(belief) + "\n")
            with open('./logs/Traj.txt', 'w+') as outfile:
                outfile.writelines(str(trajectory))
